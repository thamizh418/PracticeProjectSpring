...
public class StudioService {
	...
	public String getStudioDetailsService(StudioModel studioModel) {
		...
		if(studioDetails!=null) {
			...
			jsonResponse = "{\"status\":" + gson.toJson("Success") 
			             + ",\"message\":"+ gson.toJson("Records available")
			             + ",\"Data\":"+ gson.toJson(studioDetails)
			             + "}";
		}else {
			
			jsonResponse = "{\"status\":" + gson.toJson("Failure") 
				         + ",\"message\":"+ gson.toJson("No Records available")
				         + "}";
		}
